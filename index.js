    ...
    */
    bridgeKeyV2ToV1: (brKey) => {
        if (brKey == BridgeKey.zigbee) {
            ...
            return 6
        } else if(brKey == BridgeKey.virtual) {
            return 7
        }
    },
    /*
    ...
    */
    ...
    devidV2ToV1: (devid) => {
        ...
        if (arrDev[0] == BridgeKey.zigbee) {
            ...
            net = 6
        } else if (arrDev[0] == BridgeKey.virtual) {
            net = 7
        }
        return {
            machc: arrDevid[0],
    ...
    getDeviceTypeV1ByTypeAndTraitsV2: (type_v2, traits) => {
        ...
        switch (type_v2) {
            ...
            case DeviceTypeV2.INPUT:
                ...
                break;
            case DeviceTypeV2.INPUTV2:
                type = DeviceTypeV1.INPUTV2
                break;
            case DeviceTypeV2.COOKER:
                type = DeviceTypeV1.COOKER
                break;
            case DeviceTypeV2.LMYALELOCK:
                type = DeviceTypeV1.LMYALELOCK
                break;
            case DeviceTypeV2.VIRTUAL:
                type = DeviceTypeV1.VIRTUAL
                break;
            default:
                break;
        }
    ...
    executionToVal: (obj, type) => {
        ...
        if (obj.command == Traits.ModeActive) {
            val.modeActive = obj.params.modeActive
        }
        if (obj.command == Traits.ModeConfig) {
            val.modeConfig = obj.params.modeConfig
        }
        ...
        if (obj.command == Traits.WaterAlarm) {
            val.level = obj.params.waterAlarm ? '1' : '0';
        }
        ...
        if (obj.command == Traits.LockUnlock) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.LockUnlock;
                val.act = obj.params.act;
                val.source = obj.params.source;
                val.userId = obj.params.userId;
                val.logIndex = obj.params.logIndex;
            } else {
        ...
        if (obj.command == Traits.PinCode) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.PinCode;
                val.act = obj.params.act;
                val.name = obj.params.name;
                val.length = obj.params.length;
        ...
        if (obj.command == Traits.Battery) {
            if (type == DeviceTypeV2.SENSOR) {
                val.level = obj.params.battery;
            } else {
                val.battery = obj.params.battery;
            }
        }
        if (obj.command == Traits.Language) {
            val.language = obj.params.language;
        }
        if (obj.command == Traits.FingerPrint) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.FingerPrint;
                val.act = obj.params.act;
                val.name = obj.params.name;
                val.source = obj.params.source;
                val.userId = obj.params.userId;
        ...
        if (obj.command == Traits.ArmDisarm) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.ArmDisarm;
                val.act = obj.params.act;
                val.source = obj.params.source;
                val.userId = obj.params.userId;
                val.logIndex = obj.params.logIndex;
        ...
        if (obj.command == Traits.YaleLockLog) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.YaleLockLog;
                val.number = obj.params.number;
                val.logIndex = obj.params.logIndex;
            }
        }
        if (obj.command == Traits.YaleLockCode) {
            if (type == DeviceTypeV2.LMYALELOCK) {
                val.trait = Traits.YaleLockCode;
                val.act = obj.params.act;
                val.pinCode = obj.params.pinCode;
                val.length = obj.params.length;
                val.source = obj.params.source;
    ...
    valToStates: (device_type_v1, val) => {
        ...
        switch (device_type_v1) {
            ...
            case 5:
                ...
                if(val.act == 0 && val.state == 3) {
                    states.IrStatus = {
                        irStatus: "activeError"
                    }
                }
                if(val.act == 0 && val.state == 4) {
                    states.IrStatus = {
                        irStatus: "fullMem"
                    }
                }
                if(val.act == 0 && val.state == 5) {
                    states.IrStatus = {
                        irStatus: "learnError"
                    }
                }
            ...
            case 15:
                ...
                if (val.modeConfig != undefined) {
                    states.ModeConfig = {
                        modeConfig: parseInt(val.modeConfig)
                    }
                }
                break;
            case 25:
    ...
    traitsByDeviceTypeV1: (type_v1, net_V1) => {
        ...
        switch (type_v1) {
            ...
            case 25: 
                traits.push({name: Traits.OnOff, is_main: true})
                traits.push({name: Traits.Act, is_main: false})
                break;
            case 123:
            ...
            case 145:
                traits.push({name: Traits.LockUnlock, is_main: true})
                traits.push({name: Traits.Battery, is_main: false})
                traits.push({name: Traits.Language, is_main: false})
                traits.push({name: Traits.PinCode, is_main: false})
                traits.push({name: Traits.FingerPrint, is_main: false})
                traits.push({name: Traits.ArmDisarm, is_main: false})
                traits.push({name: Traits.YaleLockCode, is_main: false})
                traits.push({name: Traits.YaleLockLog, is_main: false})
                traits.push({name: Traits.HCTime, is_main: false})
                break;
            default:
                break;
        }
    ...
    deviceTypeV1ToV2: (type_v1) => {
        ...
        switch (type_v1) {
            ...
            case 144:
                ...
                break;
            case 145:
                type_v2 = DeviceTypeV2.LMYALELOCK
                break;
            case 146:
                type_v2 = DeviceTypeV2.VIRTUAL
                break;
            default:
                type_v2 = DeviceTypeV2.UNKNOW
                break;
    ...
    rulesRowV2ToV1: (dataRowV2) => {
        ...
        if (inV2 !== undefined && inV2.devices !== undefined) {
            ...
            if (inV2.sequenceType != undefined) {
                dataRowV1.in.sequenceType = inV2.sequenceType;
            }
            if (inV2.delay != undefined) {
                dataRowV1.in.delay = inV2.delay;
            }
        }
        if (inV2 !== undefined && inV2.schedule !== undefined) {
            dataRowV1.timeschedule.time = {}
            if (inV2.schedule.starttime != undefined) {
                if (inV2.schedule.starttime.moment !== undefined && inV2.schedule.starttime.offset !== undefined) {
                    dataRowV1.timeschedule.time.starttime = inV2.schedule.starttime
                } else {
                    dataRowV1.timeschedule.time.starttime = inV2.schedule.starttime.replace(':', '')
                }
            }
            if (inV2.schedule.endtime != undefined) {
                if (inV2.schedule.endtime.moment !== undefined && inV2.schedule.endtime.offset !== undefined) {
                    dataRowV1.timeschedule.time.endtime = inV2.schedule.endtime
                } else {
        ...
        if (inV2 != undefined && inV2.HCL != undefined) {
            dataRowV1.in.HCL = { data: {}, rp: {} }
            if (inV2.HCL.data != undefined && inV2.HCL.deviceType != undefined && inV2.HCL.traits != undefined) {
                dataRowV1.in.HCL.deviceType = Converter.getDeviceTypeV1ByTypeAndTraitsV2(inV2.HCL.deviceType, inV2.HCL.traits)
                for (let key in inV2.HCL.data) {
                    if (inV2.HCL.data.hasOwnProperty(key)) {
                        dataRowV1.in.HCL.data[key] = { lst: [] }
                        for (let i in inV2.HCL.data[key]) {
                            dataRowV1.in.HCL.data[key].lst.push(Converter.executionToVal(inV2.HCL.data[key][i], inV2.HCL.deviceType));
                        }
                    }
                }
            }
            if (inV2.HCL.repeat != undefined) {
                dataRowV1.in.HCL.rp = inV2.HCL.repeat;
            }
        }
        if (inV2 !== undefined && inV2.people !== undefined) {
            dataRowV1.in.people = inV2.people;
        }

        if (outV2 != undefined) {
            if (outV2.devices != undefined) {
                if (dataRowV1.type != 5) {
                    dataRowV1.out.dev = Converter.deviceOutV2ToV1(outV2.devices)
                } else {
                    dataRowV1.out.devices = outV2.devices.map(dev => Converter.devidV2ToV1(dev.devid))
                }
            }
            ...
            if (outV2.groups != undefined) {
                if (dataRowV1.type != 5) {
                    dataRowV1.out.groups = Converter.groupOutV2ToV1(outV2.groups)
                } else {
                    dataRowV1.out.groups = outV2.groups
                }
            }
            ...
            if (outV2.notify != undefined) {
                ...
                if (outV2.notify.smsDcom != undefined && outV2.notify.content != undefined) {
                    if (outV2.notify.before == undefined) {
                        dataRowV1.notify.smsDcom = [{content: outV2.notify.content, smsDcom: outV2.notify.smsDcom}]
                    } else {
                        dataRowV1.notify.smsDcom = [{content: outV2.notify.content, smsDcom: outV2.notify.smsDcom, before: outV2.notify.before}]
                    }
                }
                if (outV2.notify.callDcom != undefined && outV2.notify.content != undefined) {
                    if (outV2.notify.before == undefined) {
                        dataRowV1.notify.callDcom = [{content: outV2.notify.content, callDcom: outV2.notify.callDcom}]
                    } else {
                        dataRowV1.notify.callDcom = [{content: outV2.notify.content, callDcom: outV2.notify.callDcom, before: outV2.notify.before}]
                    }
                }
                if (outV2.notify.notifyTimes != undefined) {
                    dataRowV1.notify.notifyTimes = outV2.notify.notifyTimes
                }
                if (outV2.notify.notifyCycle != undefined) {
                    dataRowV1.notify.notifyCycle = outV2.notify.notifyCycle
                }
            }
        }
        return dataRowV1
    ...
    rulesRowV1ToV2: (dataRowV1) => {
        ...
        if (inV1 !== undefined && inV1.dev !== undefined) {
            ...
            if (inV1.sequenceType != undefined) {
                dataRowV2.in.sequenceType = inV1.sequenceType
            }
            if (inV1.delay != undefined) {
                dataRowV2.in.delay = inV1.delay
            }
        }

        if (inV1 !== undefined && inV1.HCL !== undefined) {
            dataRowV2.in.HCL = {};
            if (inV1.HCL.data !== undefined && inV1.HCL.deviceType !== undefined) {
                dataRowV2.in.HCL.deviceType = Converter.deviceTypeV1ToV2(inV1.HCL.deviceType)
                dataRowV2.in.HCL.traits = Converter.traitsByDeviceTypeV1(inV1.HCL.deviceType)
                dataRowV2.in.HCL.data = {}
                for (let key in inV1.HCL.data) {
                    if (!inV1.HCL.data.hasOwnProperty(key)) {
                        continue
                    }
                    dataRowV2.in.HCL.data[key] = inV1.HCL.data[key].lst.map(val => {
                        let states = Converter.valToStates(inV1.HCL.deviceType, val)
                        let execution = {}
                        Object.keys(states).map((key, index) => {
                            execution.command = key
                            execution.params = states[key]
                        })
                        return execution
                    })
                }
            }
            if (inV1.HCL.rp != undefined) {
                dataRowV2.in.HCL.repeat = inV1.HCL.rp
            }
        }

        if (inV1 !== undefined && inV1.people !== undefined) {
            dataRowV2.in.people = inV1.people
        }
        ...
        if (timescheduleV1 !== undefined) {
            ...
            if (timescheduleV1.time != undefined) {
                if(timescheduleV1.time.starttime != undefined){
                    if (timescheduleV1.time.starttime.moment !== undefined && timescheduleV1.time.starttime.offset !== undefined) {
                        dataRowV2.in.schedule.starttime = timescheduleV1.time.starttime
                    } else {
                ...
                if(timescheduleV1.time.endtime != undefined){
                    if (timescheduleV1.time.endtime.moment !== undefined && timescheduleV1.time.endtime.offset !== undefined) {
                        dataRowV2.in.schedule.endtime = timescheduleV1.time.endtime
                    } else {
        ...
        if (outV1 != undefined) {
            if (dataRowV1.type != 5) {
                ...
                if (outV1.groups != undefined && outV1.groups.constructor == Array) {
                    dataRowV2.out.groups = Converter.groupOutV1ToV2(outV1.groups)
                }
            } else {
                if (outV1.devices != undefined) {
                    dataRowV2.out.devices = outV1.devices.map(dev => ({ devid: Converter.devidV1ToV2(dev.machc, dev.devid, dev.net, dev.ord)}))
                }
                if (outV1.groups != undefined) {
                    dataRowV2.out.groups = outV1.groups
                }
            }
        }
        ...
        if (notifyV1 != undefined) {
            if (notifyV1.push != undefined) {
                ...
                if (notifyV1.smsDcom != undefined) {
                    if (notifyV1.smsDcom[0] != undefined && notifyV1.smsDcom[0].smsDcom != undefined) {
                        dataRowV2.out.notify.smsDcom = notifyV1.smsDcom[0].smsDcom
                    }
                }
                if (notifyV1.callDcom != undefined) {
                    if (notifyV1.callDcom[0] != undefined && notifyV1.callDcom[0].callDcom != undefined) {
                        dataRowV2.out.notify.callDcom = notifyV1.callDcom[0].callDcom
                    }
                }
                ...
                if (notifyV1.notifyTimes != undefined) {
                    dataRowV2.out.notify.notifyTimes = notifyV1.notifyTimes;
                }
                if (notifyV1.notifyCycle != undefined) {
                    dataRowV2.out.notify.notifyCycle = notifyV1.notifyCycle;
                }
    ...
    deviceInV2ToV1: (inV2_devices) => {
        ...
        for (let i in inV2_devices) {
            ...
            if (rowDeviceV2.sequence != undefined) {
                rowDevV1.sequence = rowDeviceV2.sequence
            }
    ...
    deviceInV1ToV2: (inV1_dev) => {
        ...
        for (let i in inV1_dev) {
            ...
            if (rowDevV1.sequence != undefined) {
                rowDevV2.sequence = rowDevV1.sequence
            }
    ...
    groupOutV2ToV1: (outV2_devices) => {
        let groups = []
        for (let i in outV2_devices) {
            let rowDeviceV2 = outV2_devices[i]
            let rowDevV1 = {groupid: rowDeviceV2.groupid, timer: rowDeviceV2.delay}
            rowDevV1.type = Converter.getDeviceTypeV1ByTypeAndTraitsV2(rowDeviceV2.type, rowDeviceV2.traits)
            if(rowDeviceV2.remoteid != undefined) {
                rowDevV1.remoteid = rowDeviceV2.remoteid
            }
            ...
            groups.push(rowDevV1)
        }
        return groups
    },

    groupOutV1ToV2: (outV1_dev) => {
        let outV2_devices = []
        for (let i in outV1_dev) {
            let rowDevV1 = outV1_dev[i]
            let rowDevV2 = {
                groupid: rowDevV1.groupid,
                delay: rowDevV1.timer
            }
            if(rowDevV1.remoteid != undefined) {
                rowDevV2.remoteid = rowDevV1.remoteid
                ...
                case 'group': {
                    ...
                    switch (objFormat.cmd) {
                        ...
                        case 'sync':
                            ...
                            for (let i in lst) {
                                if (lst[i].devices != undefined) {
                                    lst[i].devices = lst[i].devices.map(dev => Converter.devidV1ToV2(dev.machc, dev.devid, dev.net, dev.ord))
                                }
                                if (lst[i].deviceType != undefined) {
                                    const type_v1 = lst[i].deviceType
                                    lst[i].deviceType = Converter.deviceTypeV1ToV2(type_v1)
                                    lst[i].traits = Converter.traitsByDeviceTypeV1(type_v1, 1);
                                }
                                data.push(lst[i])
                            }
                            payload_new = {
                        ...
                        case 'sync': {
                            ...
                            for (let i in lstRemoteGet) {
                                ...
                                let rowSync = {
                                    ...
                                    devid: devid_new,
                                    brigde_key: Converter.bridgeKeyV1ToV2(lstRemoteGet[i].net),
        ...
        try {
            ...
            switch (cmd) {
                case 'set':
                    ...
                    switch (type) {
                        case 'advanced':
                            let net = Converter.bridgeKeyV2ToV1(data.brigde_key)
                            let mac = data.machc
                            let dev = {}
                            if (data.hash) {
                                dev = Converter.hashV2ToV1(data.hash)
                                console.log(dev)
                                delete data.hash
                            }
                            delete data.brigde_key
                            delete data.machc
                    ...
                    switch (type) {
                        case 'groups':
                            for (let i in object.data) {
                                if (object.data[i].devices != undefined) {
                                    object.data[i].devices = object.data[i].devices.map(device => Converter.devidV2ToV1(device));
                                }
                                if (object.data[i].deviceType != undefined && object.data[i].traits != undefined) {
                                    const deviceType = Converter.getDeviceTypeV1ByTypeAndTraitsV2(object.data[i].deviceType, object.data[i].traits);
                                    object.data[i].deviceType = deviceType;
                                    delete object.data[i].traits;
                                }
                            }
                ...
                case 'sync':
                    ...
                    switch (type) {
                        ...
                        case 'groups':
                            ...
                            str_cmd = 'group=lstres' + JSON.stringify({lst: object.data}) + '$end'
                            break;
                        case 'devices':
                            let dev = []
                            for (let i in data) {
                ...
                case 'get':
                    ...
                    switch (type) {
                        case 'devices_local':
                            let lst = data.map(dev => Converter.devidV2ToV1(dev));
                            str_cmd = '$dev=getconfig' + JSON.stringify({lst}) + '$end'
                            break;
                        default: break;
                    }
                    break;
    ...
    */
                ...
                case 'set':
                    switch (type) {
                        case 'devices':
                            ...
                            for (let i in data) {
                                ...
                                if(object.info != undefined){
                                    ...
                                    if (Array.isArray(object.execution)) {
                                        for (let it of object.execution) {
                                            val = {
                                                ...val,
                                                ...Converter.executionToVal(it, object.info[data[i]].type, object.info[data[i]].traits)
                                            }
                                        }
                                    } else {
                                        val = Converter.executionToVal(object.execution, object.info[data[i]].type, object.info[data[i]].traits);
                                    }
                                }
                                let devInfo = Converter.devidV2ToV1(data[i])
                                dev.push({
                        ...
                        case 'groups':
                            ...
                            str_cmd = 'group=' + cmd + JSON.stringify({"lst": groups}) + '$end'
                            break;
                        case 'rules':
                            let lst = [];
                        ...
                        case 'ota':
                            let lstOta = []
                            for (let data of object.data) {
                                lstOta.push({machc:object.data[0]["json_data"]["mac"]})
                            }
                            str_cmd = '$hc=system' + JSON.stringify({
                                act: 3, lst: lstOta
                            }) + '$end'
        ...
        try {
            ...
            switch (objFormat.type) {
                ...
                case 'dev':
                    ...
                    switch (objFormat.cmd) {
                        case 'advancedStatus': 
                            let bridge_key = Converter.bridgeKeyV1ToV2(objFormat.obj_data.net)
                            delete objFormat.obj_data.net
                            data.push({
                                bridge_key, ...objFormat.obj_data
                            })
                            payload_new = {
                                cmd: 'status',
                                reqid: reqid,
                    ...
                    break;
                case 'group':
                    let group = objFormat.obj_data.lst
                    let groups_data = []
                    switch (objFormat.cmd) {
                        case 'stt':
                            for (let i in group) {
            ...
            if (payload.control_source != undefined) {
                payload_new.control_source = payload.control_source
            }
            return payload_new;
        } catch (e) {
            throw new Error('Error: ' + e);
    ...
    statusV2ToV1: (payload) => {
        try {
            ...
            switch (type) {
                case 'people': {
                    str_cmd = `$people=stt${JSON.stringify({lst: data})}$end`
                } break;
            }
            let res = {reqid: reqid, str_cmd: str_cmd}
            return res;
